...
class VaInitialAssessments(db.Model):
    __tablename__ = "va_initial_assessments"

    va_iniassess_id: so.Mapped[uuid.UUID] = so.mapped_column(
    ...
    va_iniassess_by: so.Mapped[uuid.UUID] = so.mapped_column(
    ...
    va_iniassess_createdat: so.Mapped[datetime] = so.mapped_column(
        sa.DateTime,
        default=lambda: datetime.now(timezone.utc),
        index=True,
        nullable=False,
    )
    va_iniassess_updatedat: so.Mapped[datetime] = so.mapped_column(
        sa.DateTime,
        default=lambda: datetime.now(timezone.utc),
    ...
    def __repr__(self):
        return f"VA Initial COD -> {self.va_sid} ({self.va_iniassess_status}): {self.va_immediate_cod} | {self.va_antecedent_cod} | by {self.va_iniassess_by}"
