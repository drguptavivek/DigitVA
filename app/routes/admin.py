...
def _active_language_codes():
    """Codes of the active canonical languages, for validating form options."""
    from app.models import MasLanguages

    return set(
        db.session.scalars(
            sa.select(MasLanguages.language_code).where(
                MasLanguages.is_active.is_(True)
            )
        ).all()
    )
...
def admin_create_project():
    ...
    project = VaProjectMaster(
        ...
        reviewer_social_autopsy_enabled=bool(
            payload.get(
...
@admin.get("/api/cod-bucket-schemes/<scheme_code>/export.xlsx")
@role_required("admin")
def admin_cod_bucket_scheme_export_xlsx(scheme_code):
    if not current_user.is_admin():
        return _json_error("Admin access required.", 403)

    try:
        workbook_bytes = export_cod_bucket_scheme_xlsx(scheme_code=scheme_code)
    except LookupError:
        return _json_error("COD bucket scheme not found.", 404)
...
def admin_cod_bucket_scheme_reorder_nodes(scheme_code):
    ...
    for node in nodes[1:]:
        if (
            ...
            return _json_error("All reordered nodes must be sibling nodes of the same type.", 400)
...
def admin_cod_bucket_scheme_update_mapping(scheme_code, mapping_id):
    ...
    if (
        ...
        return _json_error("Target disease node not found for this scheme and age scope.", 404)
