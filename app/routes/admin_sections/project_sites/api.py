"""API routes for admin project-site management."""
...
def _project_site_row_query():
    ...
    ).join(VaProjectMaster, VaProjectMaster.project_id == VaProjectSites.project_id).join(
        VaSiteMaster,
        VaSiteMaster.site_id == VaProjectSites.site_id,
    )
...
def admin_project_sites():
    ...
    stmt = _project_site_row_query().where(
...
def admin_create_project_site():
    ...
    row = db.session.execute(
        _project_site_row_query().where(
            VaProjectSites.project_site_id == mapping.project_site_id
        )
    ).one()
