...
def sync_form(form_id: str):
    return run_sync_form(form_id)


def run_sync_form(form_id: str):
...
def sync_runs():
    ...
    for run in runs:
        ...
        if not (run.triggered_user_id == current_user.user_id or target in scoped_form_ids):
            continue
...
def sync_submission(va_sid: str):
    return run_sync_submission(va_sid)


def run_sync_submission(va_sid: str):
    ...
    try:
        ...
        return jsonify(
            {"message": f"Refresh started for submission {va_sid}.", "task_id": task.id}
        ), 202
    except Exception:
