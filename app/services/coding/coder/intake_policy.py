"""Coding intake policy derived from project configuration.

The `coding_intake_mode` value is stored on `va_project_master`, but the
behavior it controls belongs to the coding workflow: whether coders receive a
random allocation or can pick from eligible submissions.
"""
...
def split_form_ids_by_coding_intake_mode(
    form_ids: list[str] | set[str],
    ...
    rows = db.session.execute(
        sa.select(VaForms.form_id, VaForms.project_id).where(
