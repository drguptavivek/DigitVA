...
def _submission_cod_snapshot_mv_available() -> bool:
    """Return whether the COD snapshot materialized view currently exists."""
    return bool(
        db.session.execute(
            sa.text("SELECT to_regclass('va_submission_cod_snapshot_mv')")
        ).scalar_one()
    )
...
def dm_coded_cod_snapshot_export_csv(
    user,
    *,
    search: str = "",
    project: str = "",
    ...
    if not _submission_cod_snapshot_mv_available():
        ensure_submission_cod_snapshot_mv()
    ...
    snapshot_columns = [
        ...
        "smartva_cause2_who_bucket_provenance",
        "smartva_cause3",
        "smartva_cause3_icd",
        "smartva_cause3_who_bucket_section",
        "smartva_cause3_who_bucket",
        "smartva_cause3_who_bucket_provenance",
        "nqa_name",
        *NQA_EXPORT_DETAIL_COLUMNS,
        "nqa_score",
        "nqa_rating",
