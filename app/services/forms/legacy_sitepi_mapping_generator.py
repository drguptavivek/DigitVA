...
def va_mapping_fieldsitepi():
    mapping_name = "va_mapping_fieldsitepi"
    try:
        df = pd.read_excel(
            os.path.join(
        ...
        removeblankrows_df = df[df["category"].notna()]
        categories = removeblankrows_df["category"].unique()
        dictionaries = {}
        for category in categories:
            category_df = removeblankrows_df[removeblankrows_df["category"] == category]
            subcategory_dict = {}
