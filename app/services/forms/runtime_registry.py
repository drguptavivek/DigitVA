"""Maintain the legacy runtime form registry from active ODK site mappings.

``map_project_site_odk`` is the authoritative project/site/form mapping. This
service keeps compatibility ``va_forms`` rows aligned because submission sync,
attachment paths, permissions, and older workflow code still depend on that
runtime registry.
"""
