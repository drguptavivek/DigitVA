...
| Row `Refresh` in AG Grid Actions | `POST /api/v1/data-management/submissions/<sid>/sync` | `POST /api/v1/data-management/submissions/<va_sid>/sync` (`sync_submission`) | `run_single_submission_sync.delay(triggered_by='data-manager')` | Refresh one submission from ODK, then queue enrichment/attachments/SmartVA for that SID path. |
