...
|---|---|---|---|---|---|
| `Id10147` | Fever | `child_4_1` | `s110` | upstream value used | explicit visible WHO mapping; symptom is live |
| `Id10148_b` | Duration of fever in days | `child_4_2` | `s111` | upstream value transformed | explicit duration conversion via `DURATION_CONVERSIONS` |
| `Id10148_c` | Duration of fever in months | `child_4_2` | `s111` | upstream value transformed | explicit duration conversion via `DURATION_CONVERSIONS` |
| `Id10153` | Cough | `child_4_12` | `s121` | upstream value used | explicit visible WHO mapping |
| `Id10154_a` | Duration of cough in days | `child_4_13` | `s122` | upstream value transformed | explicit duration conversion via `DURATION_CONVERSIONS` |
| `Id10154_b` | Duration of cough in months | `child_4_13` | `s122` | upstream value transformed | explicit duration conversion via `DURATION_CONVERSIONS` |
| `Id10159` | Difficulty breathing | `child_4_16` | `s125` | upstream value used | explicit visible WHO mapping; symptom is live |
| `Id10208` | Stiff or painful neck | `child_4_28` | `s136` | upstream value used | explicit visible WHO mapping; sampled run happened to be all `No` |
| `Id10120_1` | Prepared illness duration in days | `child_1_21` | `s29` | upstream value transformed | child duration path is explicit through `who_prep.map_child_illness_duration()` |
| `Id10121` | Duration of illness in months before death | `child_1_21` | `s29` | upstream value transformed | child duration path is explicit |
| `Id10122` | Duration of illness in years before death | `child_1_21` | `s29` | upstream value transformed | child duration path is explicit |
...
| `Id10249` | Swollen legs or feet | no visible child WHO mapping | no surfaced retained child symptom | no visible retained path surfaced | downstream child swelling family exists (`s145/s146`), but this visible WHO block is not explicitly wired in this fork |
| `Id10250` | Duration of swelling lasted in days | no visible child WHO mapping | no surfaced retained child symptom | no visible retained path surfaced | downstream child swelling family exists, but this visible WHO block is not explicitly wired in this fork |
| `Id10250_b` | Duration of swelling lasted in months | no visible child WHO mapping | no surfaced retained child symptom | no visible retained path surfaced | downstream child swelling family exists, but this visible WHO block is not explicitly wired in this fork |
| `Id10251` | Both feet swollen | no visible child WHO mapping | no surfaced retained child symptom | no visible retained path surfaced | downstream child swelling family exists, but this visible WHO block is not explicitly wired in this fork |
