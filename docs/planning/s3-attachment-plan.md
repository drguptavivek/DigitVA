...
| Phase 5 | Partly done | Done: derivative rebuild from the original through Central (`repair_attachment()` on a `pending`/`stale`/`error` MP3), and readiness-based completeness for canonical repair, on-open repair and the admin backfill map (plan Finding 4 plus the retired-submission rule). Items 1 and 2 (stop downloading ordinary images; list-only availability) are **superseded** by the 2026-09-17 storage direction: DigitVA keeps its own copy of every attachment, so sync must download. Items 3 and 5 (MP3 idempotency via conditional GET at the content endpoint) hold today through sync's `If-None-Match`/304 self-heal against Central's database-backed endpoint; the S3-backed validator transition does not apply in the chosen mode. Item 6 holds by construction. Phase 5 is therefore closed. |
