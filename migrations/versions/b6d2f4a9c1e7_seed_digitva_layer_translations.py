"""Seed DigitVA-authored layer translations for twelve locales

Revision ID: b6d2f4a9c1e7
Revises: a3f7c1d9e6b4
Create Date: 2026-09-20

The DigitVA layers add questions no WHO or project workbook carries: the
consent mode and its two choices, the medical-certificate upload, the shared
"Medical and death documents" section heading, the two image-count hints, the
narration language question and its choice list, and the two ABHA fields. No
import can supply them, because no source workbook has them -- so every locale
showed 0/2 on digitva_core and abha, and one missing label held medical_records
at 32/33 and death_summary at 7/8 (the same shared heading in both).

This migration carries those strings as literals. It reads no workbook and
imports no application code, per docs/policy/migration-chaining.md.

Scope, decided 2026-09-20:
  * Twelve locales. Khasi (kha) is deliberately absent: no reliable source of
    Khasi for these strings, and a plausible-but-wrong label on a mortality
    instrument is worse than a visible gap that falls back to English.
  * ABHA is seeded for the seven Indian locales only. Ayushman Bharat Health
    Account is an Indian scheme; other locales' projects do not enable the
    `abha` extension, so a translation there would be dead weight.
  * Strings that are legitimately identical in the target language (French and
    Spanish "Hindi", "Malayalam", "Kannada", "Marathi") are not inserted. An
    untranslated string is absent from the served payload and the form falls
    back to English for it alone, which renders the correct word anyway. This
    matches the importer, which also declines to store a value equal to English.

These are machine translations and are inserted as `imported`, not `edited`:
a genuine translated workbook or a reviewer's XLIFF hand-back outranks them,
and an administrator's own edit is never overwritten. They are drafts awaiting
a speaker's review -- which is what the locale lifecycle in a3f7c1d9e6b4 is for.

Idempotent: inserts only where the locale row exists and the string is not
already present. Never overwrites. Bumps each touched locale's `version` so a
client revalidates its cached locale.
"""

import sqlalchemy as sa
from alembic import op

revision = 'b6d2f4a9c1e7'
down_revision = 'a3f7c1d9e6b4'
branch_labels = None
depends_on = None

INSTRUMENT_CODE = "WHO_2022_VA"

#: (locale_code, item_kind, item_key, field, text)
SEED_ROWS = (
    ...
    for locale_code, item_kind, item_key, field, text in SEED_ROWS:
        result = bind.execute(
            sa.text(
                ...
                SELECT :code, :locale, :kind, :key, :field, :text, 'imported', now()
...
def downgrade():
    """Remove only the untouched rows this migration wrote.

    A row an administrator has since corrected is `edited`, and a row whose
    text no longer matches the literal seeded here was changed by someone. Both
    are left standing: a downgrade must not destroy human translation work.
    """
    bind = op.get_bind()
    for locale_code, item_kind, item_key, field, text in SEED_ROWS:
        bind.execute(
