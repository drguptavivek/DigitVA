...
class TestCodingService(unittest.TestCase):
    def test_render_va_coding_page_includes_project_site_and_demo_context(self):
        ...
        with (
            ...
            patch("app.services.forms.type_resolution.va_get_form_type_code_for_form", return_value="WHO_2022_VA"),
            patch(
                "app.services.forms.category_rendering.get_category_rendering_service",
                return_value=category_service,
            ),
            patch(
                "app.services.forms.category_rendering.get_visible_category_codes",
                return_value=["vademographicdetails"],
            ),
            patch("app.services.coding.coder.workflow.is_upstream_recode", return_value=False),
            patch("app.services.coding.demo.is_demo_training_submission", return_value=True),
    ...
    def test_render_va_coding_page_queues_open_repair_for_vacode(self):
        ...
        with (
            ...
            patch("app.services.coding.coder.workflow.is_upstream_recode", return_value=False),
            patch("app.services.coding.demo.is_demo_training_submission", return_value=False),
