...
The converter and the readiness state it implies now live in the attachment
service; sync only applies what the service decided.
"""
...
class AmrConversionFailureTests(TestCase):
    def test_failure_raises_and_leaves_no_partial_output(self):
        with tempfile.TemporaryDirectory() as media_dir:
            ...
            with patch("app.services.attachment_service.subprocess.check_output", side_effect=OSError("no soxi")), \
                 patch("app.services.attachment_service.subprocess.run", side_effect=failing_run):
    ...
    def test_success_removes_source_and_returns_output(self):
        with tempfile.TemporaryDirectory() as media_dir:
            ...
            with patch("app.services.attachment_service.subprocess.check_output", return_value=b"2.0"), \
                 patch("app.services.attachment_service.subprocess.run", side_effect=ok_run):
...
class AmrSyncStateWriteTests(TestCase):
    ...
    def test_attachment_removed_on_odk_is_marked_missing(self):
        ...
        change = SimpleNamespace(
            ...
            storage_name=None,
            store_state="local",
            state_values=mark_removed_on_odk(self.SID, "narration.amr"),
        )
